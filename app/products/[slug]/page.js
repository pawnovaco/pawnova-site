...
export async function generateMetadata({params}){
 ...
 return {title:p.title,description:p.summary,alternates:{canonical:`/products/${p.slug}`},openGraph:{title:`${p.title} | Paw Nova`,description:p.summary,url:`https://pawnovaco.com/products/${p.slug}`,type:"article",images:p.image?[{url:p.image,alt:p.title}]:[]},twitter:{card:"summary_large_image",title:`${p.title} | Paw Nova`,description:p.summary,images:p.image?[p.image]:[]}};
}
export default async function ProductPage({params}){
 ...
 const schema={"@context":"https://schema.org","@type":"Article",headline:`${p.title}: Paw Nova product notes`,description:p.summary,image:p.image||undefined,author:{"@type":"Organization",name:"Paw Nova",url:"https://pawnovaco.com/about"},publisher:{"@type":"Organization",name:"Paw Nova",logo:{"@type":"ImageObject",url:"https://pawnovaco.com/pawnova-logo.png"}},dateModified:p.updated,mainEntityOfPage:{"@type":"WebPage","@id":`https://pawnovaco.com/products/${p.slug}`},about:{"@type":"Product",name:p.title,image:p.image||undefined,category:p.categoryLabel}};
 return <>
  <script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(schema)}}/>
  <section className="page-hero product-page-hero"><div className="container"><SeoBreadcrumbs items={[{name:"Home",href:"/"},{name:"Products",href:"/products"},{name:p.title,href:`/products/${p.slug}`}]}/></div><div className="container product-hero-grid">
